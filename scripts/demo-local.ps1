...
morgott в поле "To" уже стоит nikita; напишите вопрос в поле "Message" и нажмите
Send. Страница обновляется сама каждые 3 секунды: у отправленного сообщения
статус пройдёт queued -> running -> completed, и под ним появится ответ, который
дал Claude на стороне nikita (он читает папку $WorkDir только на чтение). На
странице nikita то же сообщение видно как входящее, и кнопкой Reply можно
ответить вручную. Обратное направление работает так же: с страницы nikita
отправьте вопрос morgott.
"@
Write-Host ''

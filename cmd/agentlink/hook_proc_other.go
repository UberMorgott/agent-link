...
func agentCommandLine(client string) []string {
	_, args := agentProcess(client)
	return args
}

// agentPID is the pid of the agent process this hook runs under
// (agentProcess); 0 when there is none.
func agentPID(client string) int {
	pid, _ := agentProcess(client)
	return pid
}

// agentProcess finds the agent process (agentCommandLine): its pid and
// command line, or 0 and nil.
func agentProcess(client string) (int, []string) {
	pid := os.Getppid()
	for range 10 {
		if pid <= 1 {
			return 0, nil
